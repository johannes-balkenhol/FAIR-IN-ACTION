...
# --- controlled vocabulary cache -------------------------------------------
# Generated by build_cv_cache.py on a machine WITH network access. Baked in here
# so the browser needs none — JupyterLab's CSP blocks cross-origin fetch, so an
# app that depends on the live API is broken exactly where it is used.
cvp = ROOT / "cv_cache.json"
CV = json.loads(cvp.read_text()) if cvp.exists() else {}

MODEL = {"model_version": core["model_version"], "profiles": profiles,
         "demo": DEMO, "cv": CV}
...
if CV:
    n = sum(len(v) for v in CV.values())
    print(f"  {n} cached ontology terms baked in ({len(CV)} ontologies) — app works offline\n")
else:
    print("  ⚠ no cv_cache.json — run build_cv_cache.py, or ontology lookup needs live network\n")
